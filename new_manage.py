...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    gps = gps2.GPS()
    printer = gps2.printer()
    calc_compass = dir.compass_bearing_calc()
    compass_bearing = gps2.compass_bearing()
    calc_turn_direction = dir.turn_direction_calc()
    steering_adjuster = dir.steering_adjuster()
    gps_list = dir.gps_list()
    initiate_variable = dir.initiate_variable()
    ultrasensor_controller = ultrasensor.controller()
    Kalman_Filter = multi_sensor.Kalman_Filter()
    GPS_save = gps_saver.GPS_save()
    ...
    if cfg.USE_GPS:
        V.add(gps, outputs = ['GPS/coord1', 'Kalman_bool'], threaded = True)
        V.add(compass_bearing, outputs = ['compass_bearing_1', 'ultra/left', 'ultra/middle', 'ultra/right','accel/x', 'accel/y'], threaded = True)
        V.add(initiate_variable, outputs = ['angle', 'Kalman_bool'])
        # V.add(Kalman_Filter, inputs = ['GPS/coord1', 'compass_bearing_1', 'accel/x', 'accel/y', 'Kalman_bool'], outputs = ['GPS/coord1', 'Kalman_bool'])
    ...
    if cfg.DONKEY_GYM:
        ...
        V.add(gps_list, inputs = ['GPS/coord1'], outputs = ['GPS/list'])
        V.add(GPS_save, inputs = ['GPS/coord1'])
        # V.add(printer, inputs = ['GPS/lat1'], threaded = True)
        V.add(calc_compass, inputs = ['GPS/coord1', 'GPS/list'], outputs = ['compass_bearing_2'])
        V.add(calc_turn_direction, inputs = ['compass_bearing_1', 'compass_bearing_2'], outputs = ['turn_direction', 'angle'])
        V.add(ultrasensor_controller, inputs = ['ultra/left', 'ultra/middle', 'ultra/right','angle'], outputs = ['angle'])
